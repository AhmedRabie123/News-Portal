                    ...
                    <div class="card">
                        <div class="card-body">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <div> <img src="{{ asset('uploads/' . $post_single->post_photo) }}" alt=""
                                ...
                                <select name="sub_category_id" class="form-control select2">
                                    @foreach ($sub_categories as $item)
                                        <option value="{{ $item->id }}"
                                            ...
                                            {{ $item->sub_category_name }} (Category: {{ $item->rCategory->category_name }})
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                                ...
                                <table class="table table-bordered">
                                    @foreach($existing_tags as $item)
                                       <tr>
                                         <td> {{ $item->tag_name }} </td>
                                         <td>
                                            <a href="{{ route('author_tag_delete',[$item->id, $post_single->id]) }}" onClick="return confirm('Are you sure?');">Delete</a>
                                        </td>
                                       </tr> 
                                    @endforeach
                                </table>
